<div class="container-fluid">
    <div class="row bg-title">
        <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
            <h4 class="page-title">Base Activa (Active90)</h4>
        </div>
        <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
            <ol class="breadcrumb">
                <li><a href="#">Reportes BI</a></li>
                <li class="active">Base Activa</li>
            </ol>
        </div>
    </div>
</div>
...
<div class="container">
        ...
        <div class="col-md-12">
            <section class="m-t-40">
                ...
                <form class="form-horizontal" id="report_tb_form" method="POST" action="">
                    {{ csrf_field() }}
                    @php($time = strtotime('-1 month', strtotime(date('Y-m-d'))))
                    <div class="col-md-4">
    ...
    <div class="row">
        <div class="col-md-4 col-xs-12 m-t-40">
            <div class="white-box">
                <h3 class="box-title">Base Activa (mm/aaaa) <span id="date-total">({{date('m/Y', strtotime($date->date_reg))}})</span></h3>
        ...
        <div class="row">
            <div class="col-md-12">
                <h3 class="text-center">
                    Clientes Active90 {{-- hasta el <b>{{date('m/Y', strtotime($date->date_reg))}}</b> --}}
                </h3>
    ...
    $(document).ready(function () {
        drawTable = function(){
            ...
            var tableClients = $('#tablebt').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{route('getClientsActiveBase')}}",
                    data: function (d) {
                        d.month = $('#month').val();
                        d.year = $('#year').val();
                        d.type = $('#type').val();
        ...
        $('#download').on('click', function(){
            ...
            $.ajax({
                type: "POST",
                url: "{{route('downloadClientsActiveBase')}}",
                data: data,
                dataType: "json",
                success: function(response){
                    $(".preloader").fadeOut();
