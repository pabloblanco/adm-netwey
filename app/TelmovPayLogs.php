...
class TelmovPayLogs extends Model {

  use HasFactory;

  protected $table = 'islim_api_telmovpay_logs';
 ...
 public static function saveLogBD($request = false, $data_send = false, $data_return = false, $time = false, $type = false, $error_details = false, $systemRequest = null) {
    ...
    if ($request) {
      ...
      $log->token = isset($token) ? $token : $request->bearerToken();
    ...
    if ($type != 'OK') {
      ...
      if ($request) {
        Slack::sendSlackNotification($error_details, $type, $data_send, $request, $data_return, $log->time, $systemRequest);
      } elseif ($type == 'ERROR') {
